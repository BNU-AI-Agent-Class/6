...
用于部署探活与前后端版本握手。3.3.1 发布后应返回 `status=ok`、`version=3.3.1`、`has_key=true`，但不返回真实密钥。

### `POST /api/chat`

请求仅接收 `user` 与 `assistant` 历史消息；客户端 `system` 角色会被拒绝。响应包含：
紧急帮助按钮额外发送 `emergency_help=true`，后端不调用模型、不依赖关键词，直接进入二级红灯转介。
...
| 项目 | 结果 |
|---|---|
| 后端安全回归 | 32/32 PASS，0 FAIL；包含紧急帮助按钮信号回归 |
| Render 健康检查 | 目标版本 3.3.1，发布后复验 |
...
| 本地浏览器 E2E | 真实点击紧急按钮在线红灯、断网危机兜底、普通对话与 390×844 移动视口全部通过 |
| 公网 Pages E2E | 3.3.0 普通/手工危机/降级通过；真机按钮路径暴露缺陷，3.3.1 已本地修复 |
| 真机与同伴测试 | 两台手机普通对话通过；待 3.3.1 发布后复测紧急帮助按钮 |
| 密钥扫描 | 本地 HEAD、远端 main、Git 历史与前端源码均无真实 key 模式 |

临时课程 key 风险例外、真机与同伴测试结果记录在 `checklists/上线检查清单.md`；3.3.1 上线并完成真机紧急按钮复测前，项目状态为“有条件通过”。
