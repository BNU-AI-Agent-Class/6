...
该工具是 AI，不替代心理咨询、医疗诊断或紧急服务。无需提供真实身份；每次发送时，对话会经过本项目后端并交给其配置的模型服务商生成回复，本地副本保存在当前浏览器，请勿输入敏感个人信息。可在“设置—存储”中永久清空回收站或永久清除全部本地数据。

当前主分支源码为 **3.4.0 本地候选版**；上方公网地址仍需在部署后复验，不能用历史 3.3.1 验收替代。
...
- 安全：确定性危机入口、四级安全灯、确定性边界回复、独立出口复核、客户端角色过滤和断网降级。
...
```powershell
cd backend
python -m unittest -q test_safety.py
...
- [T7 3.4.0 原始输出](./evidence/T7_3.4.0_本地实测.json)
