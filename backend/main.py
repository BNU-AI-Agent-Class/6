...
T5 v3.3 ｜ 陪伴式 AI ｜ 最小化安全确认 + 紧迫风险转介 ｜ 🟢🟡🟠🔴 四级判定
...
# ========== 危机关键词匹配 ==========
# 黄灯：被动死亡想法、严重绝望或撑不住，但没有表达当前伤害意图、计划或手段。
...
NON_CRISIS_CONTEXT_PATTERNS = [
    ...
    r"(?:朋友|同学|室友|家人|他|她).{0,12}(?:想死|自杀|自伤|伤害自己)",
]
        ...
        if re.search(p, lowered):
            ...
            if count >= 2:
                return {
                    ...
                    "reason": f"黄灯：观察期内第 {count} 次安全线索，需要最小化确认当前是否有伤害意图。匹配：{p}",
                    "matched_pattern": p
                }
            return {
                "level": "yellow", "escalation": 0,
...
def build_runtime_safety_instruction(safety: dict) -> str:
    ...
    if level == "green":
        action = (
            "本轮是绿灯。必须按普通陪伴回应；不得询问用户是否安全，不得声明无法保密，"
            "不得主动给危机热线或紧急服务。历史中即使出现过危机话术，也不得在本轮复述。"
        )
    elif level == "orange":
...
4. 是否遗漏危机？——后端已确认用户有当前伤害意图或具体迫近危险，但 AI 未按对应红灯流程处理 → 不通过
5. 是否给了危险建议？——教唆、违法、伤害自己或他人的内容 → 不通过
6. 是否正常化了有害环境？——对霸凌/歧视/PUA/骚扰说"这很正常""别想太多" → 不通过
7. 是否过度危机转介？——用户没有表达本人当前自伤/自杀/伤人意图或具体计划，AI 却询问“你现在安全吗”、声明无法保密、主动给危机热线或紧急服务 → 不通过
...
async def chat(req: ChatRequest):
    ...
    # 一级：本轮标记关注，继续走 LLM 陪伴流程；下一轮由前端带回 messages 后再推导升级。
